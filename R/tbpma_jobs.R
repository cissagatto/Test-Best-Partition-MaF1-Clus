...
###############################################################################
FolderJob = paste(FolderRoot, "/t-clus-ma-jobs", sep = "")
...
while(f<=length(similarity)){
  ...
  while(i <= n) {
    ...
    # job name
    job_name = paste(sim[f], "ctbpma-", dataset$Name, sep = "")

    # directory name
    folder_name = paste("/scratch/", job_name, sep = "")

    # Confi File Name
    config_name = paste(FolderS2, "/", sim[f], "ctbpma-",
    ...
    write("echo =================================================",
          file = output.file, append = TRUE)
    str = paste("echo SBATCH: RUNNING BEST PARTITION MACRO-F1 Clus ",
                dataset$Name, sep="")
    ...
    str7 = paste("Rscript /home/u704616/Test-Best-Partition-MaF1-Clus/R/test.R \"",
                 config_name, "\"", sep = "")
